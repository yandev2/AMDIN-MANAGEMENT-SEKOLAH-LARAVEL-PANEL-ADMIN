...
class KehadiranSiswaAnalytic extends ChartWidget
{
   ...
   protected int | string | array $columnSpan = 2;
    ...
    public function getHeading(): string|\Illuminate\Contracts\Support\Htmlable|null
    {
        ...
        return 'Analytic Kehadiran Siswa Tahun ' . $year;
    }
    ...
    public function getData(): array
    {
        ...
        $query = PresensiSiswa::selectRaw('CAST(EXTRACT(MONTH FROM tanggal) AS INTEGER) as bulan, status, COUNT(*) as total')
            ->whereRaw('EXTRACT(YEAR FROM tanggal) = ?', [$year])
            ->where('id_sekolah', auth()->user()->id_sekolah)
            ->groupByRaw('EXTRACT(MONTH FROM tanggal), status');
        ...
        $statuses = [
            'H' => ['label' => 'Hadir', 'color' =>  '#14c904',],   // Hijau
            'I' => ['label' => 'Izin', 'color' =>    '#04a2c9',],    // Biru
            'S' => ['label' => 'Sakit', 'color' =>   '#c9a504'],   // Kuning
            'A' => ['label' => 'Alpa', 'color' =>    '#c90404'],   // Kuning
        ];
